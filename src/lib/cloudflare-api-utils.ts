...
/** Name of the thin Cloudflare edge worker that Email Routing rules must target. */
export const DEFAULT_EMAIL_WORKER_NAME = "mailflare-edge";

export function getEmailWorkerName(env: CloudflareEnv): string {
	return env.CF_EMAIL_WORKER_NAME?.trim() || DEFAULT_EMAIL_WORKER_NAME;
}
