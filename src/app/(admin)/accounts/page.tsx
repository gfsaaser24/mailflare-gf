...
export default function AccountsPage() {
	...
	async function createAccount(event: React.FormEvent<HTMLFormElement>) {
		...
		setMessage(null);
		try {
			const response = await authFetch("/api/accounts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username, domainId, role, sendInvite, ...(sendInvite ? {} : { password }) }) });
			const data = (await response.json()) as AccountResponse;
			if (!response.ok) throw new Error(data.error ?? "Unable to create account");
			setUsername("");
			setPassword("");
			setCreateOpen(false);
			// Only set when the invite could not be emailed; the admin passes it on.
			setInviteUrl(data.inviteUrl ?? null);
			if (data.inviteUrl) setMessage(data.inviteMessage ?? "The invite was not emailed; copy the link below.");
			await loadAccounts();
		} catch (error) {
	...
	return <div className="space-y-6">
		...
		{inviteUrl && <code className="block truncate rounded-2xl bg-white px-4 py-3 text-sm">{inviteUrl}</code>}
		...
		<Dialog open={createOpen} onOpenChange={setCreateOpen}><DialogContent><DialogHeader><DialogTitle>Add user account</DialogTitle><DialogDescription>The user can sign in with this email and password.</DialogDescription></DialogHeader><form onSubmit={createAccount} className="space-y-4">
			...
			<label className="flex items-center gap-3 text-sm"><Checkbox checked={sendInvite} onChange={(event) => setSendInvite(event.target.checked)} />Email an invite so they set their own password</label>
			{!sendInvite && <div className="space-y-2"><Label htmlFor="account-password">Password</Label><Input id="account-password" type="password" minLength={8} value={password} onChange={(event) => setPassword(event.target.value)} required /></div>}
